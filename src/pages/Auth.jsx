...
export default function Auth() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!emailRegex.test(cleanEmail)) {
      setErrorMsg('Tolong masukkan email yang valid (contoh: nama@email.com)');
      return;
    }
      ...
      <main className="relative z-20 w-full max-w-md px-6 py-12 flex flex-col items-center mt-20">
        ...
        <div className="glass-card w-full rounded-2xl p-8 md:p-10 flex flex-col gap-8 relative overflow-hidden transition-transform duration-300">
          ...
          <div className="flex justify-center gap-5">
            <button 
              type="button"
              onClick={() => setErrorMsg('Google login is currently unavailable. Please use email.')}
              className="w-14 h-14 rounded-full bg-surface-container-lowest/40 border border-white/10 flex items-center justify-center text-on-surface-variant hover:text-primary hover:bg-white/5 transition-all duration-300 group hover:shadow-[0_0_15px_rgba(212,165,255,0.2)]"
            >
            ...
            <button 
              type="button"
              onClick={() => setErrorMsg('Apple login is currently unavailable. Please use email.')}
              className="w-14 h-14 rounded-full bg-surface-container-lowest/40 border border-white/10 flex items-center justify-center text-on-surface-variant hover:text-primary hover:bg-white/5 transition-all duration-300 group hover:shadow-[0_0_15px_rgba(212,165,255,0.2)]"
            >
              <svg className="w-6 h-6 group-hover:scale-110 transition-transform" viewBox="0 0 24 24" fill="currentColor">
