...
import { getAnime, getTVDetails, discoverAnime, getImageUrl } from '../services/tmdb';
...
export default function Anime() {
  ...
  useEffect(() => {
    if (selectedGenre) {
      ...
      discoverAnime(selectedGenre).then(async data => {
        const detailedResults = await Promise.all(
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 relative z-10">
      ...
      {!loading && (
        <>
          {renderAnimeRow("Vibrant Anime", animeList.slice(1, 10), "Anime")}
          {renderAnimeRow("Popular Picks", animeList.slice(10), "Action")}
        </>
      )}
        </>
      )}
