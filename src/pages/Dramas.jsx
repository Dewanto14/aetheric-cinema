...
import { getDramaByRegion, getTVDetails, discoverDramas, getImageUrl } from '../services/tmdb';
...
export default function Dramas() {
  ...
  useEffect(() => {
    const fetchWithDetails = async (lang) => {
      try {
        const data = await getDramaByRegion(lang);
  ...
  useEffect(() => {
    if (selectedGenre || selectedLang) {
      ...
      discoverDramas(selectedLang, selectedGenre).then(async data => {
  ...
  const renderDramaRow = (title, shows, subtext) => {
  ...
  const pageTitle = () => {
    let title = "Asian & Turkish Dramas";
    if (selectedLang) {
      title = `${regionNames[selectedLang] || 'Regional'} Dramas`;
    }
    if (selectedGenreName) {
      title = selectedLang ? `${selectedGenreName} ${title}` : `${selectedGenreName} Dramas`;
    }
    return title;
  };
      ...
      {(selectedGenre || selectedLang) ? (
        ...
        <>
          ...
          <section className="mx-4 md:mx-gutter lg:mx-container-padding mb-section-margin relative h-[50vh] md:h-[65vh] rounded-2xl md:rounded-[3rem] overflow-hidden flex flex-col justify-end shadow-2xl">
            {loading ? (
              <div className="w-full h-full rounded-lg glass-panel animate-pulse flex items-center justify-center text-primary">Memuat Dramas...</div>
            ) : korean.length > 0 && (
              <>
                <div className="absolute inset-0 z-0">
                  <div className="w-full h-full bg-cover bg-center transition-transform duration-1000 group-hover:scale-105" 
                       style={{ backgroundImage: `url('${getImageUrl(korean[0].backdrop_path)}')` }}></div>
                ...
                <div className="relative z-10 p-6 md:p-12 lg:p-container-padding max-w-2xl">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
                    <Link to={`/play/${korean[0].id}`} className="bg-gradient-to-r from-primary to-secondary text-on-primary px-6 md:px-8 py-3 rounded-full font-body-md font-bold flex items-center justify-center gap-2 bloom-hover">
                      <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1" }}>play_arrow</span>
                      Watch Now
                    </Link>
                    <Link to={`/tv/${korean[0].id}`} className="glass-panel text-on-surface px-6 md:px-8 py-3 rounded-full font-body-md font-bold flex items-center justify-center gap-2 bloom-hover">
          ...
          {!loading && (
            <>
              {renderDramaRow("Trending K-Dramas (Drakor)", korean.slice(1))}
              {renderDramaRow("Popular C-Dramas (Dracin)", chinese)}
              {renderDramaRow("Turkish Hits", turkish)}
              {renderDramaRow("Japanese Dorama", japanese)}
              {renderDramaRow("Thai Lakorn", thai)}
            </>
          )}
        </>
      )}
